...
const makeAppHead = async (
  ...
  if (username === null) {
    const title = `#CodeWrapped - Your coding year in review`;
  ...
  if (!params.stats) {
    const newHead = renderToString(
      <title>{`${username}'s #CodeWrapped`}</title>,
    );
  ...
  if (stats === null) {
    throw new Error(
      `Stats should not be null (${username}, ${backendCredentials().VITE_HOST
      })`,
    );
  }

  const usernameTitle = `${stats.username}'s #CodeWrapped`;
